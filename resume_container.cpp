...
void Resume_Container(char *buf, int len, int shim_cli_fd)
{
        int pipe_fd[2];
        pipe(pipe_fd);
        char *argv[10];
        argv[0] = "runc";
        int cur_idx = strlen("resume_container ");
        int tmp_idx = cur_idx;
        ...
        if (pid == -1) {
                ...
                if (runc_exit_code != 0) {
                        ...
                        strcpy(err_info, "resume_container_error ");
                        int len = strlen("resume_container_error ");
