...
void List_Container(char *buf, int len, int shim_cli_fd)
{
        int pipe1_fd[2];
        int pipe2_fd[2];
        pipe(pipe1_fd);
        ...
        char *argv[30];
        argv[0] = "runc";
        int cur_idx = strlen("list_container ");
        int tmp_idx = cur_idx;
        ...
        if (pid == -1) {
                ...
                if (runc_exit_code != 0) {
                        ...
                        strcpy(err_info, "list_container_error ");
                        int len = strlen("list_container_error ");
                        ...
                        close(pipe1_fd[0]);
                        char container_info[2001] = {0};
                        strcpy(container_info, "container_info ");
                        int len = strlen("container_info ");
                        int read_len = read(pipe2_fd[0], container_info + len, 2000 - len);
                        container_info[len + read_len] = '\0';
                        write(shim_cli_fd, container_info, len + read_len);
                        close(pipe2_fd[0]);
                }
        } else if (pid == 0) {
                close(pipe1_fd[0]);
